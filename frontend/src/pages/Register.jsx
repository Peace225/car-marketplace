...
export default function Register() {
  ...
  // --- LOGIQUE INSCRIPTION CLASSIQUE (EMAIL/MDP) ---
  ...
  return (
    <div className="min-h-screen w-full flex items-center justify-center bg-black relative overflow-hidden font-sans">
      {/* Background Image avec Overlay Sportif */}
      <div 
        ...
        <div className="bg-[#1a1a1a] border border-white/10 rounded-[2.5rem] p-8 shadow-2xl backdrop-blur-sm">
          ...
          <div className="text-center mb-8">
            <div className="w-16 h-16 bg-[#fb201e]/10 border border-[#fb201e]/20 rounded-2xl flex items-center justify-center mx-auto mb-4">
              <UserPlus className="text-[#fb201e]" size={32} />
            </div>
            <h1 className="text-3xl font-black text-white italic uppercase tracking-tighter leading-none">
              Créer un <span className="text-[#fb201e]">Compte</span>
            </h1>
            <p className="text-[10px] font-bold text-gray-500 uppercase tracking-widest mt-2">
              Rejoignez l'élite AutoLife
            </p>
          </div>

          {/* BOUTON GOOGLE (SVG Intégré pour affichage garanti) */}
          <button 
            onClick={handleGoogleSignIn}
            type="button"
            className="w-full bg-white text-black font-black text-[10px] py-4 rounded-xl flex items-center justify-center gap-3 hover:bg-gray-100 transition-all uppercase tracking-widest mb-6 border border-gray-200"
          >
          ...
          <form onSubmit={handleRegister} className="space-y-4">
            ...
            <button 
              type="submit" 
              className="w-full bg-[#fb201e] text-white font-black text-xs py-4 rounded-xl shadow-lg shadow-red-600/20 hover:bg-white hover:text-black transition-all flex items-center justify-center gap-2 group active:scale-[0.98] mt-6 tracking-widest uppercase"
            >
              S'INSCRIRE MAINTENANT <ArrowRight size={16} className="group-hover:translate-x-1 transition-transform" />
            </button>
          </form>
          ...
          <div className="mt-8 pt-6 border-t border-white/5 text-center">
            <p className="text-[10px] font-bold text-gray-500 uppercase tracking-widest">
              Déjà membre ?{" "}
              <Link to="/login" className="text-white hover:text-[#fb201e] transition-colors ml-1">
                Se connecter
              </Link>
            </p>
          </div>
